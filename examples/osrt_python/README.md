...
- [Python Examples](#python-examples)
  ...
  - [Example APIs for TIDL offload (or delegation) in OSRTs](#example-apis-for-tidl-offload-or-delegation-in-osrts)
    - [TfLite runtime](#tflite-runtime)
    - [ONNX runtime](#onnx-runtime)
    - [TVM](#tvm)
  - [User options for TIDL delegation](#user-options-for-tidl-delegation)
...
2. Run the following setup script on Device (script to be invoked from \<repo base> directory)
```
./dockers/SOC/PSDKRA/setup.sh
```
3. Copy below folders from PC to device where this repo is cloned
```
./model-artifacts
./models
```
4. Run inference script in the corresponding examples folder on the device and check the results, performance etc.
```
cd examples/osrt_python/tfl
python3 tflrt_delegate.py
```
...
An example call to ONNX runtime session from the python interface :
    ...
    sess = rt.InferenceSession('path_to_model', providers=EP_list, sess_options=so)
...
There are only 4 lines that are specific to TIDL offload in TVM+TIDL compilation scripts. The rest of the script is no different from a regular TVM compilation script without TIDL offload
    ...
    tidl_compiler = tidl.TIDLCompiler(delegate_options)
    
    # example of delegate options call for TVM:
...
## User options for TIDL delegation

'delegate_options' in the tfLite interpreter/ONNX inference session/TVM compiler call as outlined in above section comprise of TIDL options (required and optional) which are described below.   
...
Following options must be accessed as "advanced_options:Name" where Name is as specified in table below.

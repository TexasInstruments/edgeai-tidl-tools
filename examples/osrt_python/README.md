...
- The compilation and inference options are common across all runtimes and are defined under `examples/osrt_python/common_utils.py`
- Configuration for each model is defined under `examples/osrt_python/model_configs.py`

## Script Options
Each runtimes example script can be provided with certain options. Use `--help` with the script to see all the options allowed for the script.

#### ONNX RT
`-c / --compile` : Run TIDL Model Compilation\
`-d / --disable_offload` : Disable TIDL offload. Also delete previously compiled model artifacts. Default: False\
`-m / --models` : List of models to run. These models names are the keys defined in examples/osrt_python/model_configs.py . Default: Default out-of-box models\
`-n / --ncpus` : Number of threads to run in parallel. Only applicable for PC. Default: x86: Max CPU, aarch64: 1\
`-o / --graph_optimize` : Run on-the-fly graph optimization using \<repo_path>\/osrt-model-tools  . Default: False
...
#### TVM/Neo-AI RT
`--no-offload` : Disable TIDL offload. Also delete previously compiled model artifacts. Default: False\
`--num_bits` : Number of bit to compile model in. Default: 8\
`--num_subgraphs` : Max allowed subgraphs. Default: 16\
`--num_subgraphs` : Max allowed subgraphs. Default: 16\
`--pc-inference` : Compile for inference on PC. Default: False\
`--num_calib_images` : Number of images to use for calibration. Default: 4
...
```
2. Run Inference on PC without TIDL offload -  User can test the inference in host emulation mode without using any delegation to TI Delegate. Running with this option will delete the compiled artifacts from \<repo base>\/model-artifacts 
```
cd examples/osrt_python/ort
...
- Options are common across runtimes and are specified under `examples/osrt_python/common_utils` . Any runtime specific requirements/constraints in terms of options are called out in below documentation. 

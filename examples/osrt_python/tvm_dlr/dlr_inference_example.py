...
def preprocess_for_mxnet_mobilenetv3(image_path):
    ...
    for mean, scale, ch in zip(
        [128.0, 128.0, 128.0], [0.0078125, 0.0078125, 0.0078125], range(img.shape[2])
    ):
        img[:, :, ch] = (img.astype("float32")[:, :, ch] - mean) * scale
...
def model_create_and_run(
    model_dir, model_input_name, preprocess_func, postprocess_func, mIdx
):
...
if platform.machine() == "aarch64":
    model_output_directory = (
        "../../../model-artifacts/cl-dlr-timm_mobilenetv3_large_100_device/artifacts"
    )
    model_create_and_run(
        model_output_directory,
        "input.1",

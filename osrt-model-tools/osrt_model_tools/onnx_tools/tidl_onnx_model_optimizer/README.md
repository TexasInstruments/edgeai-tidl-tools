...
| 3 | convert_concat_unsupported_axis_to_channel | TIDL only supports concat on channel, height, or width axis (axes 1, 2, 3 in NCHW format). This function converts Concat layers with unsupported axes (e.g., batch axis 0) to Concat on channel axis by adding Transpose operations before and after the Concat, preserving functional equivalence | True |

...
cp $TARGET_FS_PATH/home/root/required_libs/* $TARGET_FS_PATH/usr/lib/
ln -s $TARGET_FS_PATH/usr/lib/libonnxruntime.so $TARGET_FS_PATH/usr/lib/libonnxruntime.so.1.7.0

echo "export the following vars with correc value in target machine"
echo "export TIDL_TOOLS_PATH=${temp}"
echo "export OPENCV_INSTALL_DIR="

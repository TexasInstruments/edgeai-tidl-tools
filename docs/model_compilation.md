...
| `advanced_options:high_resolution_optimization` | Enable high resolution optimization for improving performance on high resolution models | 0-1 (Integer) | 0 | This option enables "Super Tiling" where-in feature-maps in layers are processed in chunks instead of complete data for better memory optimization  |
| `advanced_options:high_resolution_group_info_file` | Provide group information file for creating supertiling groups. | Valid file path | "" | This option lets the user manually specify "Super Tiling" group information, which TIDL uses to generate tiling configurations so that layers are processed in chunks rather than as complete data, improving memory optimization. Refer to (./manual_supertiling_groups.md) for a comprehensive guide |
...
| `advanced_options:c7x_firmware_version` | Version of C7x firmware to use | String | 11_02_21_00 | For specific firmware compatibility |

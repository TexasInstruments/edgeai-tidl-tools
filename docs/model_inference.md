...
| `advanced_options:trace_base_name` | Base path prefix for all trace and performance CSV files written when `debug_level >= 1`. Trace files are named `<trace_base_name>_<subgraph_name>_*`. The parent directory must exist before running inference. | Valid path prefix string < 512 characters | /tmp/tidl_trace | Currently only applicable for ONNXRT and TFLiteRT |

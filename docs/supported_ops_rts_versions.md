...
| 30 | GridSample | TIDL_GridSampleLayer | <ul> <li> Only nearest & bilinear mode is supported </li><li> Only zero padding mode is supported </li><li> Only 2D grid indices are supported </li> <li>Nearest Neighbor may have poor accuracy in certain cases due to the rounding algorithm discrepancies between TIDL and ONNX</li></ul> | 
...
| 4 | Mul/Sub/Add/Div | TIDL_EltWiseLayer | <ul> <li> Constant tensor in Add/Mul/Sub/Div requires input dimensions of that layer to be present as part of the network </li><li> Only 2 inputs are supported in Add/Mul/Sub/Div layers </li><li> Number of non-singleton variable input dimensions in Add/Mul/Sub/Div must be less than <= 6 </li><li> The variable inputs in Add/Mul/Div layer must of be same dimensions or broadcast-able </li><li> Both inputs as variable are supported in Sub only if firmware version >= 10_01_06_00 </li><li> Broadcasting on all dimensions except for the batch dimension is supported for Add/Mul</li><li> 1D vector dimension should match with channel or width dimension </li></ul> | 

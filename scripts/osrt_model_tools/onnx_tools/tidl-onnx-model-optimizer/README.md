...
| 4 | convert_reducemean_to_matmul | The ReduceMean layer is replaced with the cascaded multiple layers, e.g., "Reshape + MatMul + Reshape". Assume that the number of dimes of the input tensor to ReduceMean is 4 (B, C, H, W) and the attribute, "axes" of ReduceMean should be [2], [3] or [2, 3] | False |

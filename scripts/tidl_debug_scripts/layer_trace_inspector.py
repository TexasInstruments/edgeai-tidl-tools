...
class VisualizationUtils:
    def sideBySidePlots(self, goldenBuffer: np.ndarray, quantBuffer: np.ndarray):
        ...
        (ONNX output) and a quantized buffer (TIDL output).
        """
    ...
    def plotCombinedHistograms(self, goldenBuffer: np.ndarray, quantBuffer: np.ndarray):
        """
        Plots combined histograms for two input buffers: a golden reference buffer(ONNX Output) and a quantized buffer(TIDL Output).
        """
        fig = plt.figure()
...
class LayerTraceInspector(VisualizationUtils):
    """
    LayerTraceInspector is a utility class for visualizing and analyzing layer traces
    from TIDL and ONNX models. It provides methods to map traces between the two
    frameworks, generate plots for individual or multiple layers, and summarize
    network-level errors.
    """

    def __init__(
        self, onnx_trace_folder: str, tidl_trace_folder: str, layer_info_path: str
    ):
        self.onnx_trace_folder = onnx_trace_folder
        self.tidl_trace_folder = tidl_trace_folder
        self.layer_info_path = layer_info_path
        self.tidl_onnx_trace_mapping = self.getTraces()
        ...
        for entry in entries:
            ...
            if len(onnx_trace_path) > 0 and len(tidl_trace_path) > 0:
                onnx_trace_path = os.path.join(
                    self.onnx_trace_folder, onnx_trace_path[0]
                )
    ...
    def singlePlot(self):
        ...
        with st.sidebar:
            option = st.selectbox(
                "Pick TIDL Trace to compare", self.tidl_onnx_trace_mapping.keys()
            )
        ...
        if genGraph:
            ...
            goldenBuffer = np.fromfile(
                os.path.join(self.onnx_trace_folder, optionPathFloat), dtype=np.float32
            )
            quantBuffer = np.fromfile(
                os.path.join(self.tidl_trace_folder, optionPath), dtype=np.float32
            )
    ...
    def network_error_summary(self):
        ...
        for idx in range(numLayers):
            tidl = os.path.join(
                self.tidl_trace_folder,
                self.tidl_onnx_trace_mapping[dropdown_list[idx]][1],
            )
            golden = os.path.join(
                self.onnx_trace_folder,
                self.tidl_onnx_trace_mapping[dropdown_list[idx]][0],
            )
    ...
    def multiplot(self):
        ...
        with st.sidebar:
            options = st.multiselect(
                "Pick set of TIDL Traces to compare", drop_down_list
            )
        ...
        for option in options:
            if True:
                with st.expander(f"Plots for {option}"):
                    ...
                    genGraph = False
                    goldenBuffer = np.fromfile(
                        self.onnx_trace_folder + os.path.sep + optionPathFloat,
                        dtype=np.float32,
                    )
                    quantBuffer = np.fromfile(
                        self.tidl_trace_folder + os.path.sep + optionPath,
                        dtype=np.float32,
                    )
                    if len(goldenBuffer) != len(quantBuffer):
                        st.write(f"Unable to compare {option}")
                        st.write(
                            f"{self.tidl_onnx_trace_mapping[option][1]} & {self.tidl_onnx_trace_mapping[option][0]}"
                        )
                        continue
...
if __name__ == "__main__":
    ...
    layer_trace_inspector = LayerTraceInspector(
        args.traceONNX, args.traceTIDL, args.traceInfo
    )
    ...
    pg = st.navigation(
        [
            ...
            st.Page(
                layer_trace_inspector.network_error_summary,

...
from tidl_onnx_model_optimizer.ops import test_optimizers, get_optimizers
import numpy as np
import onnxruntime
import os
...
name1 = [inp.name for inp in session1.get_outputs()]
name2 = [inp.name for inp in session2.get_outputs()]

for i in range(len(output1)):
    print(f"Absolute error obtained in the onnx optimization for output {i} ({name1[i]} -> {name2[i]}) is : {(output1[i] - output2[i]).mean()}")
    print(f"Percentage error obtained in the onnx optimization for output {i} ({name1[i]} -> {name2[i]}) is : {100*abs(output1[i] - output2[i]).mean()/(output1[i].max()-output1[i].min())}%")
